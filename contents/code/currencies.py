# -*- coding: utf-8 -*-
#   Copyright 2009 Thomas Olsen <tanghus@gmail.com>
#
#
...
# Source: http://www.iso.org/iso/support/faqs/faqs_widely_used_standards/widely_used_standards_other/currency_codes/currency_codes_list-1.htm (with slight modifications to reduce length)
#
class Currency:
  code = "XXX"
  country = ""
  ...
  def __init__(self, code="", numeric="999", name="", countries=[], symbol=None):
    self.code = code
...
CURRENCY['COP'] = Currency(code='COP', numeric='170', name='Colombian Peso', countries=['COSTA RICA'])
...
CURRENCY['GBP'] = Currency(code='GBP', numeric='826', name='Pound Sterling', symbol="£  ", countries=['UNITED KINGDOM'])
...
CURRENCY['CLP'] = Currency(code='CLP', numeric='152', name='Chilean Peso', countries=['CHILE'])
...
CURRENCY['EUR'] = Currency(code='EUR', numeric='978', name='Euro', symbol="€  ", countries=['ANDORRA', 'AUSTRIA', 'BELGIUM', 'FINLAND', 'FRANCE', 'FRENCH GUIANA', 'FRENCH SOUTHERN TERRITORIES', 'GERMANY', 'GREECE', 'GUADELOUPE', 'IRELAND', 'ITALY', 'LUXEMBOURG', 'MARTINIQUE', 'MAYOTTE', 'MONACO', 'MONTENEGRO', 'NETHERLANDS', 'PORTUGAL', 'R.UNION', 'SAINT PIERRE AND MIQUELON', 'SAN MARINO', 'SLOVENIA', 'SPAIN'])
...
CURRENCY['USD'] = Currency(code='USD', numeric='840', name='US Dollar', symbol=" $ ", countries=['AMERICAN SAMOA', 'BRITISH INDIAN OCEAN TERRITORY', 'ECUADOR', 'GUAM', 'MARSHALL ISLANDS', 'MICRONESIA', 'NORTHERN MARIANA ISLANDS', 'PALAU', 'PUERTO RICO', 'TIMOR-LESTE', 'TURKS AND CAICOS ISLANDS', 'UNITED STATES MINOR OUTLYING ISLANDS', 'VIRGIN ISLANDS (BRITISH)', 'VIRGIN ISLANDS (U.S.)'])
